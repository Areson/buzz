...
/// Built-in agent templates for the Create Agent wizard. Static data, never
/// stored — selecting a template prefills the create-agent form.
pub fn builtin_agent_templates() -> Vec<AgentTemplate> {
    BUILT_IN_PERSONAS
        .iter()
        .map(|persona| AgentTemplate {
...
/// Merge pass over the stored personas. Built-in personas are no longer
...
pub fn ensure_persona_is_active(
    ...
    if !persona.is_active {
        return Err(format!(
            "{} is not available for new agents.",
